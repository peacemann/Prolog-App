...
def insert(title, director, year, genre, rate, actor):
    myQuery="test('{0}','{1}',{2},'{3}',{4},'{5}')".format(title,director,year,genre,rate,actor)
...
def search(title="", director="", year="",genre="", rate="",actor=""):
    otherdict={}
    myQuery="test("
    ...
    if(director==""):
        ...
        myQuery += "'{0}',".format(director)
        otherdict['Director']= director
    if(year==""):
        myQuery += "Year,"
    else:
...
def update(myDict, title, director, year, genre, rate, actor):
    myQuery="test('{0}','{1}',{2},'{3}',{4},'{5}')".format(myDict['Title'],myDict['Director'],myDict['Year'],myDict['Genre'],myDict['Rate'],myDict['Actor'])
